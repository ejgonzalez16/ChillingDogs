    ...
    public Cliente findByCedula(String cedula){
        for (Map.Entry<Integer, Cliente> entry : data.entrySet()) {
            if (entry.getValue().getCedula().equals(cedula)) {
                return entry.getValue(); // Retorna el cliente encontrado
            }
        }
        return null;
    }
