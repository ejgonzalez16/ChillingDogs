...
public class TablaClientesTest {
    ...
    ComponenteProbando_elementoProbando_cosaProbando
     */
...
//        chromeOptions.addArguments("--headless"); // Ejecuta el navegador en modo headless (sin interfaz gráfica)
    ...
    @Test
    public void ClientesTest_deleteCliente_listSize(){
        // Given
        ...
        WebElement deleteButton = rows.get(0).findElement(By.className("btn-danger"));
        deleteButton.click();
        wait.until(ExpectedConditions.alertIsPresent());
        driver.switchTo().alert().accept();
        wait.until(ExpectedConditions.numberOfElementsToBe(By.tagName("tr"), initialSize - 1));
    ...
    @Test
    public void ClientesTest_deleteCliente_emptyList(){
        // Given
        driver.get(BASE_URL + "/buscar");
        wait.until(ExpectedConditions.presenceOfElementLocated(By.tagName("tbody")));
        ...
        for (WebElement row : rows) {
            WebElement deleteButton = row.findElement(By.className("btn-danger"));
            deleteButton.click();
            wait.until(ExpectedConditions.alertIsPresent());
            driver.switchTo().alert().accept();
        }
        ...
        // Then
        rows = driver.findElements(By.tagName("tr"));
        Assertions.assertThat(rows.size()).isEqualTo(0);
    }
